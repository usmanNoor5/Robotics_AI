...
# Use a default index name if ES_INDEX_NAME is not set to avoid runtime errors
index_name = os.getenv("ES_INDEX_NAME", "ragnbot-docs")
if not os.getenv("ES_INDEX_NAME"):
    logger.warning(f"Environment variable ES_INDEX_NAME not set; using default index name '{index_name}'.")
