#!/usr/bin/env bash
# Set OpenCode model in team.local.json. Zero Claude tokens.
# Usage: cmd-opencode-model.sh [model] [role-id|all]
...
OC_ROLE_LIST=$(jq -r '.roles[]? | select(.model=="opencode") | .id' "$TEAM_CONFIG" 2>/dev/null || true)
...
if [[ -z "$SELECTED_MODEL" ]]; then
  ...
  CURRENT=$(echo "$MERGED" | jq -r '.roles[]? | select(.model=="opencode") | .opencode_model // empty' 2>/dev/null | head -1)
  OC_ROLES_INLINE=$(echo "$OC_ROLE_LIST" | tr '\n' ',' | sed 's/,$//')
  echo "Current : ${CURRENT:-default}   Roles: ${OC_ROLES_INLINE:-(none)}"
  echo ""
...
if [[ "$TARGET" == "all" ]]; then
  readarray -t TARGETS <<< "$OC_ROLE_LIST"
else
  TARGETS=("$TARGET")
fi

if [[ ${#TARGETS[@]} -eq 0 || -z "${TARGETS[0]}" ]]; then
  echo "❌ No opencode roles found in team.json."
  exit 1
fi
    ...
    --arg id "$ROLE_ID" --arg m "$SELECTED_MODEL" '
      .roles = (
        ...
        if any($roles[]; .id == $id) then
          [$roles[] | if .id == $id then .opencode_model = $m else . end]
        else
          $roles + [{"id": $id, "opencode_model": $m}]
        end
      )
    ')
done
