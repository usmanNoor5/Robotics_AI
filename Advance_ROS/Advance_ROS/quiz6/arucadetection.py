...
def aruco_display(corners, ids, rejected, image):
	if len(corners) > 0:
		...
		for (markerCorner, markerID) in zip(corners, ids):
			# Fix: renamed from 'corners' to avoid shadowing the parameter
...
cap = cv2.VideoCapture(0, cv2.CAP_V4L2)  # HP True Vision FHD built-in
...
while cap.isOpened():
	...
	# Fix: check if frame was captured successfully
	if not ret:
