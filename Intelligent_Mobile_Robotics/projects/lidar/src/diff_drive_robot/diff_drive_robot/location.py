...
class Location:
    ...
    def facing_point(self, x, y):
        ...
        if None in (cx, cy, current_heading):
            return False
        n = self.necessary_heading(cx, cy, x, y)
    ...
    def faster_left(self, x, y):
        ...
        if None in (cx, cy, current_heading):
            return False
        return math.radians(current_heading) - self.necessary_heading(cx, cy, x, y)
...
# current x, y; target x,y
    def necessary_heading(self,cx, cy, tx, ty):
        # if abs(tx-cx)<5.0 or abs(ty-cy)<5.0:
        #     return 0
